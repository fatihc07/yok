...
- Obtain the OİS attendance write/read contract or a verification endpoint. The current write endpoint can return `err: 0` without supplying a durable record identifier or a readable result, so it cannot prove that a weekly attendance value was applied.
