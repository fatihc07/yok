...
- The controlled PSK 301 test bridge uses `hafta: "1"` for every test write because that is the administrator-provided working OİS test payload even where its date is not the first displayed week. This is deliberately limited to the temporary test bridge; live courses retain their calculated week values.
...
- Run the controlled test bridge only with the authorized PSK 301 test course, then confirm the OİS test daily-attendance view records a theory session's QR-confirmed student as `saat: 0` and absent student as `saat: 6`, with the administrator-provided `hafta: "1"` field. Weekly-report confirmation requires OİS’s end-of-day transfer or a read API.
