...
- Confirm the corrected OİS lesson-slot mapping against the controlled test course: for one absent and one QR-confirmed student it must send `saat: <lesson-hours>` and `Usaat: 0`, respectively, with the selected academic date and `hafta: 1` for PSK 301's only recurring slot. The current write endpoint can return `err: 0` without supplying a durable record identifier or a readable result, so it cannot itself prove that a weekly attendance value was applied.
