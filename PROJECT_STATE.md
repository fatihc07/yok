...
- OİS attendance serialization follows the administrator's validated contract for every course: `ders_id` is numeric, while instructor ID, section, week, and student number are strings. Absent students use `saat: <OİS lesson hours>` and QR-confirmed students use `Usaat: 0`.
