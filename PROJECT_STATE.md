...
- The controlled OİS test course now separates course-owner identity from helper-instructor identity. Course discovery stays tied to the owner, while the test attendance write's `kullanici_id` is configurable through `OBS_TEST_ATTENDANCE_INSTRUCTOR_ID`; remove this diagnostic bridge before production.
