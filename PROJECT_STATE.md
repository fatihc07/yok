...
- OİS attendance fields follow the administrator-provided common write convention for every course/session: a non-QR-confirmed student sends `saat: <lesson-hours>`, while a QR-confirmed student sends `Usaat: 0`. The selected academic week and OİS program/session metadata remain independent of that per-student absence field mapping.
...
- Every course derives `hafta` from the selected academic week. When an OİS program row supplies a distinct attendance/API week code, that OİS-provided code is retained and sent instead. No course-specific fixed test-week override exists.
...
- The controlled OİS test fixture preserves the administrator's explicitly validated 13.07.2026 packet (`ders_id` as text, `hafta: 1`, absent `saat: 2`, present `Usaat: 0`) solely to validate the test-host contract. It does not alter the general academic-calendar mapping used by normal courses.
...
- Confirm the restored general OİS payload against the controlled test course after the stale local/hosted history cleanup: for one absent and one QR-confirmed student it must send `saat: <lesson-hours>` and `Usaat: 0`, respectively, with the selected date and academic week. The current write endpoint can return `err: 0` without supplying a durable record identifier or a readable result, so it cannot itself prove that a weekly attendance value was applied.
- Controlled browser tests on 2026-09-11 sent both (a) two absent students at 2 hours and (b) the administrator's mixed form: one `saat:2` absence plus one `Usaat:0` QR-confirmed student. Both used course 27063, 13.07.2026, API week 1. OİS test returned HTTP 200 / `err:0` in both cases, while the week-5 entry and July daily report remained 0. This is a test-host persistence/contract issue, not a client week, QR, or field-mapping issue.
