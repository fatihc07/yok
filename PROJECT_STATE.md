...
Kampüs Yoklama v1.2.0 — local QR attendance MVP with an OİS write adapter.
...
- Normal OİS attendance payloads use the authenticated instructor's `kullanici_id`. The temporary PSK 301 controlled test is the sole exception: the course-owner login is retained for lookup but its CRM attendance request uses `OBS_TEST_ATTENDANCE_INSTRUCTOR_ID` (the authorized helper instructor), allowing the two identities to be tested independently.
- The OİS preview's instructor identifier is read directly from the generated attendance payload, not from the account that opened the course. It therefore displays the exact `kullanici_id` that will be sent to OİS.
...
- A delivery report without OİS diagnostics now explicitly shows the server's dry-run/write-disabled message and delivery mode instead of falsely implying that an OİS response was unavailable.
- When OİS writing is enabled but credentials/configuration are missing, the instructor report now safely names the missing Railway variable keys and retains the non-secret attendance payload; credential values are never exposed.
...
- Release version `v1.2.0` is visible on the instructor login screen and throughout the instructor interface header, providing a direct deployment/freshness check after Railway rollout.
