...
// hoca farklıdır. Bu yalnızca bu test dersinin CRM isteğindeki
// `kullanici_id` alanını değiştirir; normal derslerde giriş yapan hoca gider.
...
function controlledTestCourseFor(instructorId, season, semester) {
  ...
  return {
    ...
    attendanceInstructorId: CONTROLLED_TEST_ATTENDANCE_INSTRUCTOR_ID,
...
function courseParticipation(instructorId, course) {
  const meetingKeys = new Set((course.meetings || []).map(meeting => attendanceHistoryKey(instructorId, course, meeting)));
  const acceptedEvents = audit.filter(item => (
    item.actorId === String(instructorId)
    && item.metrics?.accepted === true
    && meetingKeys.has(item.metrics?.attendanceKey)
  ));
  ...
  return {
    ...
    acceptedAttempts: acceptedEvents.length,
    acceptedSessions: new Set(acceptedEvents.map(item => item.metrics.attendanceKey)).size,
  };
}
...
async function readOisAssignedCourses(instructorId, season, semester) {
  ...
  // `hocaders` yanıtında ders saati anahtarları OİS sürümüne göre küçük
  // farklılıklar gösterebiliyor. Bunlar doğrudan ders kaydında saklanır;
  // öğretim elemanı hiçbir yerde ders saati girişi yapmaz.
  const numericField = (row, keys) => {
    const entry = Object.entries(row || {}).find(([key, value]) => (
      keys.includes(normalizeKey(key))
      && value !== null && value !== undefined && String(value).trim() !== ''
    ));
    if (!entry) return null;
    const value = Number(String(entry[1]).trim().replace(',', '.'));
    return Number.isFinite(value) && value >= 0 ? value : null;
  };
  const courseHours = row => ({
    theoreticalHours: numericField(row, ['teorik', 'teori', 'teoriksaat', 'teorikderssaati', 'teoriderssaati']),
    practicalHours: numericField(row, ['pratik', 'uygulama', 'pratiksaat', 'uygulamasaati', 'pratikderssaati', 'uygulamaderssaati']),
    laboratoryHours: numericField(row, ['laboratuvar', 'lab', 'laboratuvarsaati', 'labsaat', 'laboratuvarderssaati']),
  });
  ...
  const courses = list.map(row => ({
    id: Number(row.ders_id ?? row.id),
    code: row.ders_kodu ?? row.ders_kod ?? row.code ?? '—',
    title: row.ders_adi ?? row.ders_ad ?? row.title ?? 'İsimsiz ders',
    section: String(row.section ?? row.sube ?? '1'),
    academicYear: season,
    semester: Number(semester),
    term: `${season} · ${{ 1:'Güz', 2:'Bahar', 3:'Yaz' }[Number(semester)] || semester}`,
    ...courseHours(row),
    raw: row,
  }));
...
app.get('/api/instructors/me/participation', auth, (req, res) => {
  ...
  res.json({
    ...
    summary: {
      ...
      acceptedAttempts: rows.reduce((total, row) => total + row.acceptedAttempts, 0),
      acceptedSessions: rows.reduce((total, row) => total + row.acceptedSessions, 0),
      attendedTotal,
      possibleTotal,
      participationRate: possibleTotal ? attendedTotal / possibleTotal : null,
    },
...
app.get('/api/sessions/:id/ois-preview', auth, (req, res) => { const session = getSession(req, res); if (!session) return; const sessionInstructor = instructors[session.instructorId], target = oisWriteTarget(session.course), payload = attendancePayload(session), payloadInstructorId = String(payload.ders[0].kullanici_id); res.json({ deliveryTarget: { environment: target.environment, label: target.label, endpoint: target.base || null }, instructor: { id: payloadInstructorId, name: payloadInstructorId === String(sessionInstructor.id) ? sessionInstructor.name : 'Yardımcı öğretim elemanı' }, course: { id: session.course.id, code: session.course.code, title: session.course.title, academicYear: session.course.academicYear, semester: session.course.semester, section: session.course.section, theoreticalHours: session.course.theoreticalHours, practicalHours: session.course.practicalHours, laboratoryHours: session.course.laboratoryHours }, week: session.week, apiWeek: session.apiWeek, date: session.meetingDate, type: session.meetingType, absenceHours: session.absenceHours, absentField: session.absentField, presentField: session.presentField, payload }); });
...
app.post('/api/sessions/:id/end', auth, async (req, res) => { const session = getSession(req, res); if (!session) return; if (session.state !== 'CLASS_OPEN') return res.status(410).json({ error: 'Ders oturumu zaten bitmiş' }); session.state = 'ENDED'; session.qrOpen = false; session.challenge = null; const oisPayload = attendancePayload(session); try { const delivery = await sendToOis(oisPayload, session.course); session.oisDelivery = delivery; const verified = Boolean(delivery.sent && delivery.verified); const accepted = Boolean(delivery.accepted); if (verified) { const history = attendanceHistory[session.historyKey] || { opens: session.openNumber, present: [] }; history.opens = Math.max(Number(history.opens) || 0, session.openNumber); history.present = session.present.map(student => ({ no: String(student.no), name: student.name, checkedAt: student.checkedAt || new Date().toISOString() })); history.enrolled = (courseStudents[session.course.id] || []).length; history.lastSentAt = new Date().toISOString(); history.verified = true; attendanceHistory[session.historyKey] = history; saveAttendanceHistory(); } const summary = attendanceDeliverySummary(session, verified); for (const pairing of tvPairings.values()) if (pairing.sessionId === session.id) pairing.status = 'ENDED'; const actor = auditContext(req, 'instructor', req.instructorId, instructors[req.instructorId]?.name); event(verified ? 'OİS yoklama gönderildi' : accepted ? 'OİS isteği kabul edildi (doğrulanmadı)' : 'OİS yoklama paketi hazırlandı', `${session.course.code} · hafta ${session.week} · ${summary.attended}/${summary.total} öğrenci`, { ...actor, category: 'attendance', metrics: { attendanceKey: session.historyKey, courseId: session.course.id, courseCode: session.course.code, meetingId: session.meetingId, week: session.week, total: summary.total, attended: summary.attended, absent: summary.absent, sent: verified, accepted, verified } }); res.json({ ok: true, oisPayload, delivery, summary, openNumber: session.openNumber, maxOpens: MAX_QR_OPENS, opensRemaining: MAX_QR_OPENS - session.openNumber }); } catch (error) { session.state = 'CLASS_OPEN'; event('OİS gönderimi başarısız', `${session.course.code} · ${error.message}`, { ...auditContext(req, 'instructor', req.instructorId, instructors[req.instructorId]?.name), category: 'attendance', metrics: { courseId: session.course.id, courseCode: session.course.code, meetingId: session.meetingId, week: session.week } }); res.status(502).json({ error: 'OİS gönderimi başarısız; ders oturumu yeniden açık bırakıldı.', detail: error.message, oisPayload, oisDebug: error.oisDebug || null }); } });
